...
// vocabulary, (2) execution FAILS CLOSED to handoff on a mandate failure and never
...
const validator = (spec: ValidatorSpec, locked = true, steer = false): CompiledValidator =>
  ({ key: "k", spec, locked, steer, provenance: prov("x") });

describe("compile: constraints project out of the resolved object", () => {
  it("compiles an active mandate constraint, carrying its lock and steer flag", () => {
    ...
    expect(compiled[0]).toMatchObject({ key: "compliance.language.official", locked: true, steer: true });
  });

  it("compiles nothing for an inactive, unlocked constraint", () => {
    const resolved = resolve(
      treeOf({ tenant: [engine({ key: "style.no-links", scope: "tenant", kind: "constraint", behaviour: "default", check: { type: "forbid-pattern", pattern: "url" }, defaultValue: false })] }),
    );
    expect(compileValidators(resolved)).toHaveLength(0);
  });
});

describe("execute: mandate failures fail closed to handoff", () => {
...
describe("steer: a steer-needed constraint is in BOTH the prompt and the validators (M1)", () => {
  it("renders a steering line only for steer:true, and validates either way", () => {
    const resolved = resolve(
      treeOf({
        timeshift: [engine({ key: "handoff.required", scope: "timeshift", kind: "constraint", behaviour: "mandate", steer: false, check: { type: "handoff-available" }, defaultValue: true })],
